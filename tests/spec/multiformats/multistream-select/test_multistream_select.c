...
static void multistream_select_spec_check_message(
    const uint8_t *payload,
    size_t payload_len,
    const uint8_t *expected,
    size_t expected_len)
{
    uint8_t encoded[LIBP2P_MULTISTREAM_SELECT_MAX_ENCODED_MESSAGE_BYTES] = {0U};
...
static void multistream_select_spec_test_canonical_messages(void)
{
    ...
    multistream_select_spec_check_message(
...
static void multistream_select_spec_test_pipelined_selection_vector(void)
{
...
static void multistream_select_spec_test_ls_response_vector(void)
{
    ...
    uint8_t response[LIBP2P_MULTISTREAM_SELECT_MAX_ENCODED_MESSAGE_BYTES] = {0U};
    ...
    assert(
        libp2p_multistream_select_message_encode(
            ...
            response,
            sizeof(response),
            &response_len) == LIBP2P_MULTISTREAM_SELECT_OK);
    assert(response_len == (sizeof(expected_response) - 1U));
    assert(memcmp(response, expected_response, response_len) == 0);

    assert(
        libp2p_multistream_select_ls_response_payload_decode(payload, payload_len, NULL, NULL, &count) ==
        LIBP2P_MULTISTREAM_SELECT_OK);
    assert(count == 4U);
}

static void multistream_select_spec_test_empty_ls_response_vector(void)
{
    ...
    assert(
        libp2p_multistream_select_message_encode(NULL, 0U, response, sizeof(response), &response_len) ==
        LIBP2P_MULTISTREAM_SELECT_OK);
    assert(response_len == (sizeof(expected_response) - 1U));
    assert(memcmp(response, expected_response, response_len) == 0);

    assert(
        libp2p_multistream_select_ls_response_payload_decode(NULL, 0U, NULL, NULL, &count) ==
        LIBP2P_MULTISTREAM_SELECT_OK);
    assert(count == 0U);
}
...
int main(void)
{
    multistream_select_spec_test_canonical_messages();
    multistream_select_spec_test_pipelined_selection_vector();
    multistream_select_spec_test_ls_response_vector();
    multistream_select_spec_test_empty_ls_response_vector();
    return 0;
}
