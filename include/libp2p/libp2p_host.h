 ...
 *     (void)host;
 *     (void)direction;
 *     return libp2p_host_stream_set_user_data(stream, protocol_user_data);
 * }
 *
 * static libp2p_host_err_t identify_event(
 *     libp2p_host_t *host,
 *     libp2p_host_stream_t *stream,
 ...
 *     {
 *         return app_identify_read_step(host, stream);
 *     }
 *     return LIBP2P_HOST_OK;
 * }
 *
 * libp2p_host_config_t config;
 * libp2p_host_protocol_t identify;
 * libp2p_host_t *host = NULL;
 * size_t storage_len = 0U;
 * size_t storage_align = 0U;
 * void *storage = app_aligned_storage;
 *
 * (void)libp2p_host_config_default(&config);
 * config.identity = app_identity;
 * config.listen_multiaddr = listen_addr;
 * config.listen_multiaddr_len = listen_addr_len;
 * config.transport = libp2p_host_quic_transport();
 * config.transport_config = &quic_service_config;
 *
 * (void)libp2p_host_storage_size(&config, &storage_len);
 * (void)libp2p_host_storage_align(&storage_align);
 * (void)libp2p_host_init(storage, storage_len, &config, &host);
 *
 * identify.id = identify_id;
 * identify.id_len = sizeof(identify_id) - 1U;
 * identify.on_open = identify_open;
 * identify.on_event = identify_event;
 * identify.user_data = &app_identify_state;
 *
 * (void)libp2p_host_handle(host, &identify);
 * (void)libp2p_host_start(host);
 *
 * for (;;)
 * {
 *     libp2p_host_ready_t ready = app_wait_for_host(host);
 ...
 *     {
 *         app_ping_mark_ready(protocol_user_data, stream);
 *     }
 *     return libp2p_host_stream_set_user_data(stream, protocol_user_data);
 * }
 *
 * static libp2p_host_err_t ping_event(
 *     libp2p_host_t *host,
 *     libp2p_host_stream_t *stream,
 *     libp2p_host_protocol_event_kind_t kind,
 *     void *protocol_user_data)
 ...
 *     int fin = 0;
 *
 *     if (kind == LIBP2P_HOST_PROTOCOL_EVENT_WRITABLE)
 *     {
 *         return libp2p_host_stream_write(
 *             host,
 *             stream,
 *             app_ping_payload(protocol_user_data),
 *             32U,
 *             0,
 *             &accepted);
 *     }
 *     if (kind == LIBP2P_HOST_PROTOCOL_EVENT_READABLE)
 *     {
 *         libp2p_host_err_t err = libp2p_host_stream_read(
 *             host,
 *             stream,
 *             buf,
 *             sizeof(buf),
 *             &len,
 *             &fin);
 *         if ((err == LIBP2P_HOST_OK) && (len == 32U))
 *         {
 *             app_ping_observe_response(protocol_user_data, buf);
 *             return libp2p_host_stream_finish(host, stream);
 *         }
 *         return err;
 *     }
 *     return LIBP2P_HOST_OK;
 * }
 *
 * libp2p_host_protocol_t ping;
 * libp2p_host_dial_t *dial = NULL;
 * libp2p_host_stream_open_t *open = NULL;
 * libp2p_host_event_t event;
 *
 * ping.id = ping_id;
 * ping.id_len = sizeof(ping_id) - 1U;
 * ping.on_open = ping_open;
 * ping.on_event = ping_event;
 * ping.user_data = &app_ping_state;
 *
 * (void)libp2p_host_handle(host, &ping);
 * (void)libp2p_host_start(host);
 *
 * (void)libp2p_host_dial(host, peer_addr, peer_addr_len, &peer_ctx, &dial);
 * (void)libp2p_host_drive(host, now_us, LIBP2P_HOST_READY_APP, NULL);
 *
 * while (libp2p_host_next_event(host, &event) == LIBP2P_HOST_OK)
 * {
 *     if (event.type == LIBP2P_HOST_EVENT_CONN_ESTABLISHED)
 *     {
 *         (void)libp2p_host_open_stream(
 *             host,
 *             event.conn,
 *             ping_id,
 *             sizeof(ping_id) - 1U,
 ...
 *             &open);
 *     }
 *     else if (event.type == LIBP2P_HOST_EVENT_DIAL_FAILED)
 *     {
 ...
 *
 * protocols[0].id = identify_id;
 * protocols[0].id_len = sizeof(identify_id) - 1U;
 * protocols[0].on_open = identify_open;
 * protocols[0].on_event = identify_event;
 * protocols[0].user_data = &identify_state;
 *
 * protocols[1].id = ping_id;
 * protocols[1].id_len = sizeof(ping_id) - 1U;
 * protocols[1].on_open = ping_open;
 * protocols[1].on_event = ping_event;
 * protocols[1].user_data = &ping_state;
 *
 ...
 * libp2p_host_ready_t ready = 0U;
 *
 * (void)libp2p_host_fd(host, &fd);
 * (void)libp2p_host_io_interest(host, &interest);
 * (void)libp2p_host_next_deadline(host, &deadline);
 *
 * if ((interest & LIBP2P_HOST_INTEREST_READ) != 0U)
 * {
 *     app_event_loop_watch_read(fd);
 * }
 * if ((interest & LIBP2P_HOST_INTEREST_WRITE) != 0U)
 * {
 *     app_event_loop_watch_write(fd);
 * }
 * app_event_loop_watch_timer(deadline);
 *
